...
p = Path("src/components/design/production-studio-advanced.tsx")
s = p.read_text()
...
needle = '''type DragState = {
  ids: string[];
  startPoint: { x: number; y: number };
  startLayers: DesignLayer[];
};
'''
replacement = needle + '''type TransformState =
...
needle = 'const allowedFonts = ["Arial", "Impact", "Georgia", "Courier New", "Times New Roman"];\n'
replacement = needle + '''const transformHandles: Array<{ handle: TransformHandle; x: -1 | 1; y: -1 | 1 }> = [
...
needle = '  const [openingVersion, setOpeningVersion] = useState<number | null>(null);\n'
replacement = needle + '  const [inspectorOpen, setInspectorOpen] = useState(false);\n'
assert needle in s
s = s.replace(needle, replacement, 1)

needle = '  const dragRef = useRef<DragState | null>(null);\n'
replacement = needle + '  const transformRef = useRef<TransformState | null>(null);\n'
assert needle in s
s = s.replace(needle, replacement, 1)

needle = '  const canUngroup = selectedLayers.some((layer) => Boolean(layer.groupId));\n'
replacement = needle + '''  const handleRadius = Math.max(3, Math.min(7, Math.min(size.width, size.height) / 70));
...
needle = '''  function startDrag(event: ReactPointerEvent<SVGGElement>, layer: DesignLayer) {
'''
insert = '''  function startResize(event: ReactPointerEvent<SVGGElement>, layer: DesignLayer, handle: TransformHandle) {
...
old = '''  function drag(event: ReactPointerEvent<SVGSVGElement>) {
...
'''
new = '''  function drag(event: ReactPointerEvent<SVGSVGElement>) {
    const point = canvasPoint(event.clientX, event.clientY);
    if (!point) return;
...
needle = '''  const deviceTone = device === "connected"
'''
renderer = '''  function renderSelectionInspector() {
    return (
...
'''
assert needle in s
s = s.replace(needle, renderer + needle, 1)

old = '''            <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => setShowGrid(!showGrid)}><Grid3X3 size={16} /> {showGrid ? "Grid on" : "Grid off"}</Button><Button variant="outline" onClick={() => setSnap(!snap)}>{snap ? "Snap 5 mm" : "Free move"}</Button></div>'''
...
old = '''                  {selectedSet.has(layer.id) ? <rect x={-layer.width / 2} y={-layer.height / 2} width={layer.width} height={layer.height} fill="none" stroke={primarySelectedId === layer.id ? "#0284c7" : "#059669"} strokeWidth="1" vectorEffect="non-scaling-stroke" strokeDasharray={primarySelectedId === layer.id ? "4 3" : "2 2"} /> : null}
'''
new = old + '''                  {selectedIds.length === 1 && primarySelectedId === layer.id && !layer.locked ? (
...
s = s.replace('The editor stays unmirrored for accurate positioning. Dragging any selected member moves the unlocked selection together.', 'The editor stays unmirrored for accurate positioning. Drag a selected layer to move it; one unlocked layer also shows resize and rotation handles.')

start_marker = '''          <section className="panel p-4">
            <div className="flex items-center justify-between gap-2"><h3 className="font-semibold">Selection</h3><span className="text-xs text-slate-500">{selectedIds.length} selected</span></div>'''
end_marker = '''          <section className="panel p-4">
            <h3 className="font-semibold">Material and cut setup</h3>'''
start = s.index(start_marker, s.index('<aside className="space-y-4">', s.index('<main className="panel')))
end = s.index(end_marker, start)
s = s[:start] + '          <div className="hidden xl:block">{renderSelectionInspector()}</div>\n\n' + s[end:]
