...
async function applyProviderRefundState(
  refund: Pick<PaymentRefund, "id" | "shopId" | "paymentId" | "processedAt">,
  data: unknown,
) {
  ...
  await prisma.paymentRefund.update({
    ...
    data: {
      ...
      processedAt: status === PaymentRefundStatus.PROCESSED ? refund.processedAt ?? now : refund.processedAt,
    },
  });
  await syncPaymentRefundStatus(refund.shopId, refund.paymentId);
  return prisma.paymentRefund.findFirstOrThrow({ where: { id: refund.id, shopId: refund.shopId } });
}

export async function listPaymentRefundsForPayment(shopId: string, paymentId: string) {
...
export async function requestPaymentRefund(input: {
  ...
  try {
    ...
    return prisma.paymentRefund.findFirstOrThrow({ where: { id: reserved.id, shopId: input.shopId } });
  }
...
export async function applyPaystackRefundWebhook(input: {
  ...
  const current = await prisma.paymentRefund.findFirstOrThrow({ where: { id: refund.id, shopId: refund.shopId } });
  ...
  await prisma.paymentRefund.update({
    ...
    data: {
      ...
      failedAt: status === PaymentRefundStatus.FAILED ? now : null,
      processedAt: status === PaymentRefundStatus.PROCESSED ? current.processedAt ?? now : current.processedAt,
    },
  });
  await syncPaymentRefundStatus(refund.shopId, refund.paymentId);
...
export async function retryPaymentRefundWithBankDetails(input: {
  ...
  if (!bankId || !/^\d{6,20}$/.test(accountNumber)) throw new Error("REFUND_BANK_DETAILS_INVALID");
