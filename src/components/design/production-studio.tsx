...
export function DesignStudio({
  ...
  function drag(event: ReactPointerEvent<SVGSVGElement>) {
    ...
    const point = canvasPoint(event.clientX, event.clientY);
    if (!point) return;
    setLayers((items) => items.map((layer) => layer.id === active.id
      ? clampLayerToSheet({ ...layer, x: snapValue(point.x - active.offsetX), y: snapValue(point.y - active.offsetY) }, size)
      : layer));
  }
  ...
  function nudgeLayer(event: ReactKeyboardEvent<SVGGElement>, layer: DesignLayer) {
    ...
    updateLayer(layer.id, { x: layer.x + move.x, y: layer.y + move.y });
  }
  ...
  function duplicateLayer(layer: DesignLayer) {
    addLayer({ ...layer, id: id(), name: `${layer.name} copy`.slice(0, 120), x: layer.x + 5, y: layer.y + 5 });
  }
      ...
      <div className="grid gap-4 xl:grid-cols-[300px_minmax(0,1fr)_330px]">
          ...
          <section className="panel p-4">
            ...
            <div className="mt-3 max-h-72 space-y-2 overflow-y-auto">
              {[...layers].reverse().map((layer) => (
                <div key={layer.id} className={`rounded-lg border p-2 ${selectedId === layer.id ? "border-[var(--shop-primary)] bg-emerald-50" : "border-[#ded8cd] bg-white"}`}>
                  <button type="button" className="block w-full truncate text-left text-sm font-semibold" onClick={() => setSelectedId(layer.id)}>{layer.name}</button>
                  <div className="mt-2 flex gap-1">
                    ...
                    <button type="button" aria-label="Delete" className="ml-auto rounded p-1 text-red-600 hover:bg-red-50" onClick={() => { checkpoint(layers.filter((item) => item.id !== layer.id)); if (selectedId === layer.id) setSelectedId(null); }}><Trash2 size={15} /></button>
                  </div>
                </div>
              ))}
        ...
        <main className="panel min-w-0 overflow-hidden">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#ded8cd] bg-white px-4 py-3">
            ...
            <div className="flex gap-2"><Button variant="outline" onClick={() => setShowGrid(!showGrid)}><Grid3X3 size={16} /> {showGrid ? "Grid on" : "Grid off"}</Button><Button variant="outline" onClick={() => setSnap(!snap)}>{snap ? "Snap 5 mm" : "Free move"}</Button></div>
          </div>
            ...
            >
              ...
              {layers.map((layer) => layer.visible ? (
                ...
                >
                  ...
                  {selectedId === layer.id ? <rect x={-layer.width / 2} y={-layer.height / 2} width={layer.width} height={layer.height} fill="none" stroke="#0284c7" strokeWidth="1" vectorEffect="non-scaling-stroke" strokeDasharray="4 3" /> : null}
                </g>
              ) : null)}
          ...
          <div className="border-t border-[#ded8cd] bg-white px-4 py-3 text-xs text-slate-600">The editor stays unmirrored for accurate positioning. The exported or printed output applies the mirror setting automatically.</div>
        </main>
