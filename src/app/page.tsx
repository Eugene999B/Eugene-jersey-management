...
const capabilities = [
  ...
  { icon: Palette, label: "Create", detail: "Products, services, job workflows and optional production tools." },
  { icon: Boxes, label: "Control", detail: "Live stock, purchasing, supplier orders and multi-location control." },
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f4f7fb] text-[#07111f]">
      ...
      <header className="relative z-20 border-b border-slate-200/80 bg-white/75 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
          <Link href="/" className="flex min-w-0 items-center gap-3">
            <Image src="/brand/esm-mark.svg" alt="Eugene Shop Management" width={44} height={44} priority />
            <div className="min-w-0"><p className="truncate text-sm font-extrabold tracking-tight sm:text-base">Eugene Shop Management</p><p className="hidden text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400 sm:block">Commerce • production • control</p></div>
          </Link>
      ...
      <section className="relative mx-auto grid max-w-7xl gap-12 px-4 pb-16 pt-14 sm:px-6 sm:pt-20 lg:grid-cols-[1.02fr_0.98fr] lg:items-center lg:px-8 lg:pb-24 lg:pt-24">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full border border-cyan-200 bg-cyan-50 px-3 py-2 text-xs font-bold uppercase tracking-[0.14em] text-cyan-800"><Sparkles size={14} /> Built for serious growing businesses</div>
          ...
          <p className="mt-7 max-w-2xl text-base leading-8 text-slate-600 sm:text-lg">Run sales, orders, products, services, customer accounts, payments, stock, purchasing and optional production workflows from one secure business platform.</p>
        ...
        <div className="relative mx-auto w-full max-w-2xl">
          ...
          <div className="relative overflow-hidden rounded-[32px] border border-white/80 bg-[#07111f] p-3 shadow-[0_40px_120px_rgba(7,17,31,0.28)]">
            <div className="flex items-center justify-between border-b border-white/10 px-4 py-3 text-white"><div className="flex items-center gap-3"><Image src="/brand/esm-mark.svg" alt="" width={34} height={34} /><div><p className="text-xs text-white/45">Live business workspace</p><p className="text-sm font-semibold">Operations overview</p></div></div><span className="rounded-full bg-emerald-400/15 px-3 py-1 text-xs font-semibold text-emerald-300">Secure session</span></div>
            <div className="grid gap-3 bg-[#eef3f8] p-3 sm:grid-cols-[155px_1fr]">
              <aside className="hidden rounded-2xl bg-white p-3 sm:block"><p className="px-2 text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400">Workspace</p><div className="mt-3 space-y-1">{["Overview", "Sales & POS", "Orders", "Items & services", "Customers", "Reports"].map((item, index) => <div key={item} className={`rounded-xl px-3 py-2.5 text-xs font-semibold ${index === 0 ? "bg-[#07111f] text-white" : "text-slate-500"}`}>{item}</div>)}</div></aside>
              <div className="space-y-3">
                ...
                <div className="rounded-2xl bg-white p-4 shadow-sm"><div className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-400">Operations pulse</p><p className="mt-1 text-base font-bold">From customer request to completion</p></div><Palette size={20} className="text-violet-600" /></div><div className="mt-4 grid grid-cols-3 gap-2">{["New", "In progress", "Ready"].map((item, index) => <div key={item} className="rounded-xl bg-slate-100 p-3"><p className="text-[10px] font-bold text-slate-500">{item}</p><div className="mt-3 space-y-2"><div className={`h-2 rounded-full ${index === 0 ? "w-5/6 bg-cyan-400" : index === 1 ? "w-2/3 bg-violet-500" : "w-1/2 bg-emerald-400"}`} /><div className="h-2 w-3/5 rounded-full bg-slate-200" /></div></div>)}</div></div>
              </div>
            </div>
          </div>
        </div>
