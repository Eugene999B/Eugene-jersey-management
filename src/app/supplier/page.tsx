    ...
    <main style={style} className="min-h-screen bg-slate-100">
      ...
      <section className="mx-auto max-w-7xl space-y-5 px-5 py-6">
        <div className="grid gap-4 md:grid-cols-3">
          <div className="panel p-4"><p className="text-xs font-semibold uppercase text-slate-500">Open orders</p><p className="mt-2 text-3xl font-semibold">{supplier.supplierOrders.filter((order) => order.status !== "RECEIVED" && order.status !== "CANCELLED").length}</p></div>
          <div className="panel p-4"><p className="text-xs font-semibold uppercase text-slate-500">Total order value</p><p className="mt-2 text-3xl font-semibold">{currency(supplier.supplierOrders.reduce((sum, order) => sum + Number(order.totalAmount), 0), supplier.shop.currency)}</p></div>
          <div className="panel p-4"><p className="text-xs font-semibold uppercase text-slate-500">Lead time</p><p className="mt-2 text-3xl font-semibold">{supplier.leadTimeDays} days</p></div>
        </div>
          ...
          <div className="divide-y divide-slate-200 bg-white">
            {supplier.supplierOrders.map((order) => (
              <article key={order.id} className="grid gap-4 p-4 lg:grid-cols-[1fr_180px]">
                <div>
                  <div className="flex flex-wrap items-center gap-2"><p className="font-semibold">{order.orderNumber}</p><Badge tone={order.status === "RECEIVED" ? "green" : "orange"}>{titleCase(order.status)}</Badge></div>
                ...
                <form action={acknowledgeSupplierOrderAction}>
                  ...
                  <Button variant="outline" className="w-full" disabled={order.status !== "SENT"}><PackageCheck size={16} /> Acknowledge</Button>
                </form>
              </article>
            ))}
