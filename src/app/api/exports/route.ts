...
async function exportData(module: ExportModule, shopId: string, filters: ExportFilters): Promise<TableExport> {
  ...
  if (module === "closing") {
    ...
    return {
      ...
      columns: ["Date", "Closed by", "Sales", "Debt collected", "Debt cash", "Debt card", "Debt momo", "Expected cash", "Manual cash", "Difference", "Status"],
      rows: closings.map((closing) => [
        ...
        currency(closing.debtCollections.toString(), shop.currency),
        currency(closing.debtCash.toString(), shop.currency),
        currency(closing.debtCard.toString(), shop.currency),
        currency(closing.debtMomo.toString(), shop.currency),
