...
import { createPlatformWorkerAction, togglePlatformWorkerAction } from "@/app/admin/actions";
...
export default async function StaffPage({ searchParams }: StaffPageProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {params.error && params.error !== "worker-exists" ? <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-800">The worker action could not be completed. Check required fields and permissions.</div> : null}

      <section className="grid gap-5 xl:grid-cols-[0.82fr_1.18fr]">
        <div className="panel p-5">
          <h2 className="text-xl font-semibold">Create platform worker</h2><p className="mt-2 text-sm text-slate-500">Workers receive only the platform areas selected below. Leave no ambiguity about their responsibility.</p>
        ...
        <div className="panel overflow-hidden">
          <div className="border-b border-[#ded8cd] p-5"><h2 className="text-xl font-semibold">Platform team</h2><p className="mt-1 text-sm text-slate-500">{workers.length} administrator account{workers.length === 1 ? "" : "s"}. Your own account cannot be suspended here.</p></div>
          <div className="divide-y divide-[#ded8cd] bg-white">{workers.map((worker) => {
            ...
            return <article key={worker.id} className="p-4 text-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><p className="font-semibold">{worker.name}</p><Badge tone={worker.isActive ? "green" : "red"}>{worker.isActive ? "Active" : "Suspended"}</Badge>{worker.id === session.id ? <Badge tone="blue">Current account</Badge> : null}</div><p className="mt-1 text-slate-500">{worker.email}</p><p className="mt-1 text-xs font-semibold text-slate-600">ID: {worker.adminLoginId ?? "Not assigned"}</p><p className="text-xs text-slate-500">{worker.staffTitle ?? "Admin worker"}{worker.department ? ` · ${worker.department}` : ""}</p></div><form action={togglePlatformWorkerAction}><input type="hidden" name="userId" value={worker.id} /><Button disabled={worker.id === session.id} variant={worker.isActive ? "outline" : "primary"} className="min-h-8 px-2 py-1 text-xs">{worker.id === session.id ? "Protected" : worker.isActive ? "Suspend" : "Activate"}</Button></form></div>
