...
async function requireShopAdministration() {
  const session = await requireRole(permissions.superAdmin);
  const user = await prisma.user.findUnique({ where: { id: session.id }, select: { adminPermissions: true } });
  const values = Array.isArray(user?.adminPermissions) ? user.adminPermissions.map(String) : [];
  const recognised = values.filter((value) => platformPermissionValues.includes(value as (typeof platformPermissionValues)[number]));
  if (recognised.length > 0 && !recognised.includes("shops")) redirect("/admin?error=permission");
  return session;
}

export async function createSecureShopAction(formData: FormData) {
  const session = await requireShopAdministration();
  ...
  const shop = await prisma.$transaction(async (tx) => {
    const createdShop = await tx.shop.create({
      data: {
        ...
        staffLoginId: proposedStaffLoginId,
  ...
  await audit({
    ...
    entityType: "Shop",
    entityId: shop.id,
    metadata: { ownerEmail: parsed.data.ownerEmail, planTier: parsed.data.planTier, credentialDelivery: "out-of-band" },
  });
