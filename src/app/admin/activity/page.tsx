...
export default async function ActivityPage({ searchParams }: ActivityPageProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <form className="panel grid gap-3 p-4 sm:grid-cols-[1fr_190px_auto]"><label className="relative"><span className="sr-only">Search activity</span><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={17} /><input className="field pl-10" name="q" defaultValue={params.q ?? ""} placeholder="Search action, user, shop or entity" /></label><label className="relative"><span className="sr-only">Activity scope</span><Filter className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={17} /><select className="field pl-10" name="scope" defaultValue={scope}><option value="all">All activity</option><option value="platform">Platform only</option><option value="tenant">Tenant activity</option><option value="security">Authentication</option></select></label><button type="submit" className="rounded-xl bg-slate-950 px-5 text-sm font-semibold text-white">Apply filters</button></form>
