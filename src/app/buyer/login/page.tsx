...
type Props = {
  searchParams?: Promise<{ next?: string; error?: string; securityChanged?: string }>;
};
    ...
    <main className="min-h-screen overflow-hidden bg-[#07111f] text-white">
      ...
      <section className="relative mx-auto grid max-w-7xl gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[0.82fr_1.18fr] lg:items-start lg:py-12">
        ...
        <div className="rounded-[32px] border border-white/10 bg-[#f4f7fb] p-4 text-[#07111f] shadow-[0_35px_120px_rgba(0,0,0,0.28)] sm:p-6 lg:p-8">
          <div className="mb-5"><p className="text-xs font-black uppercase tracking-[0.17em] text-violet-600">Customer gateway</p><h2 className="mt-2 text-3xl font-black tracking-[-0.04em]">Sign in or create an account.</h2><p className="mt-2 text-sm leading-6 text-slate-600">Existing buyers sign in below. New customers can open the dedicated registration page immediately.</p></div>
          <Link href={`/buyer/register?next=${encodeURIComponent(next)}`} className="mb-5 flex min-h-14 w-full items-center justify-between gap-3 rounded-2xl border border-cyan-200 bg-cyan-50 px-4 text-left text-cyan-950 shadow-sm transition hover:border-cyan-300 hover:bg-cyan-100"><span className="flex items-center gap-3"><span className="rounded-xl bg-white p-2.5 text-cyan-700"><UserPlus size={19} /></span><span><span className="block text-sm font-bold">Create a buyer account</span><span className="mt-0.5 block text-xs text-cyan-900/70">Verify your phone and continue to checkout.</span></span></span><ArrowRight size={18} /></Link>
