...
const errors: Record<string, string> = {
  ...
  send: "The verification email could not be sent. Check the email provider and verified sending domain, then try again.",
  rate: "Too many attempts. Wait a few minutes before trying again.",
  code: "That email code is incorrect, expired or has already been used.",
    ...
    <main className="min-h-screen bg-[#07111f] px-4 py-8 text-white sm:px-6">
      <section className="mx-auto max-w-2xl">
        ...
        <div className="mt-8 rounded-[32px] border border-white/10 bg-[#f4f7fb] p-5 text-[#07111f] shadow-[0_35px_120px_rgba(0,0,0,0.28)] sm:p-8">
          ...
          <p className="mt-3 text-sm leading-7 text-slate-600">Phone verification already protects your buyer account. Email verification adds another trusted contact channel for account notices and receipts.</p>
          ...
          {params.sent ? <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-semibold text-emerald-800">Verification email sent. Check Inbox, Promotions and Spam, then enter the six-digit code below.</div> : null}

          {verified ? (
            <div className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-5 text-emerald-900"><ShieldCheck size={24} /><h2 className="mt-3 text-xl font-bold">Email verified</h2><p className="mt-2 text-sm leading-6">This email is now verified for the current buyer account.</p><Link href={next} className="mt-4 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-700 px-4 text-sm font-semibold text-white">Continue <ArrowRight size={16} /></Link></div>
          ) : buyer.email ? (
