...
export async function resetPasswordAction(formData: FormData) {
  ...
  if (parsed.data.phone && parsed.data.code) {
    ...
    try {
      await enforceRateLimit({ key: `password-reset-verify:${phoneRateKey(phone)}`, limit: 8, windowSeconds: 15 * 60 });
    } catch {
    ...
    if (!user || !user.isActive || user.phone !== phone) redirect(`/reset-password?error=expired&phone=${encodeURIComponent(phone)}`);
  ...
  if (!reset || reset.usedAt || reset.expiresAt < new Date() || !reset.user.isActive) redirect("/reset-password?error=expired");
