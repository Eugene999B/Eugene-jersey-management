...
import { CustomerProductionRequestStatus, Role } from "@prisma/client";
...
const PRODUCTION_ACTION_ROLES: ReadonlySet<Role> = new Set([Role.OWNER, Role.MANAGER, Role.DESIGNER]);
...
export default async function CustomerProductionDashboard({ searchParams }: { searchParams?: Promise<Record<string, string | undefined>> }) {
  ...
  const canOpenProductionStock = session.role === Role.OWNER || session.role === Role.MANAGER;
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        eyebrow="Customers and online approvals"
        title="Custom production requests"
        description="Review customer artwork, issue a quoted concept preview, wait for explicit approval and verified deposit, then move the linked order through production, ready and completion."
        actions={<>
          <LinkButton href="/dashboard/orders" variant="outline"><PackageCheck size={16} /> Orders</LinkButton>
          {canManageProduction ? <LinkButton href="/dashboard/designs" variant="outline"><Shirt size={16} /> Design Studio</LinkButton> : null}
          {canOpenProductionStock ? <LinkButton href="/dashboard/production-stock" variant="outline">Stock & costing</LinkButton> : null}
        </>}
      />
      ...
      {!canManageProduction ? <FeedbackState state="info" title="Read-only production view" description={`Your ${titleCase(session.role)} role can review customer requests, approval status and payment milestones. Only an owner, manager or designer can issue previews or advance production stages.`} /> : null}
      ...
      <div className="space-y-4">
        {requests.map((request) => {
          ...
          const canQuote = canManageProduction && QUOTABLE_STATUSES.has(request.status);
          return <article key={request.id} className="panel overflow-hidden">
            ...
            <div className="grid gap-5 p-4 sm:p-5 xl:grid-cols-[0.85fr_1.15fr]">
              ...
              <div className="space-y-4">{canQuote ? <form action={quoteCustomerProductionRequestAction} className="rounded-xl border border-cyan-200 bg-cyan-50 p-4"><div className="flex items-center gap-2"><Sparkles size={17} className="text-cyan-800" /><h3 className="font-bold text-cyan-950">Issue quoted preview</h3></div><p className="mt-1 text-xs text-cyan-900">The generated concept shows garment, placement, text and number. Final cutter/press production still uses the reviewed Design Studio production brief.</p><input type="hidden" name="requestId" value={request.id} /><div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="text-sm font-semibold">Quoted total ({shop.currency})<input className="field mt-1" name="quotedTotal" type="number" min="0.01" step="0.01" required defaultValue={request.quotedTotal ? Number(request.quotedTotal) : ""} /></label><label className="text-sm font-semibold">Deposit amount<input className="field mt-1" name="depositAmount" type="number" min="0" step="0.01" required defaultValue={request.depositAmount ? Number(request.depositAmount) : ""} /></label><label className="text-sm font-semibold sm:col-span-2">Preview / quote note<textarea className="field mt-1 min-h-24" name="previewNote" minLength={3} maxLength={1000} required defaultValue={request.previewNote ?? "Confirm garment size, placement, spelling, number, colours and artwork before approving."} /></label><label className="text-sm font-semibold">Quote expires<input className="field mt-1" name="quoteExpiresAt" type="date" /></label><div className="flex items-end"><Button>Send preview v{request.previewVersion + 1}</Button></div></div></form> : request.previewSvg ? <div className="overflow-hidden rounded-xl border border-[#ded8cd] bg-slate-50"><img className="mx-auto max-h-[460px] w-full object-contain" src={`data:image/svg+xml;charset=utf-8,${encodeURIComponent(request.previewSvg)}`} alt={`Preview version ${request.previewVersion}`} /></div> : null}
                ...
                {canManageProduction && request.status === CustomerProductionRequestStatus.DEPOSIT_PAID ? <form action={advanceCustomerProductionAction}><input type="hidden" name="requestId" value={request.id} /><input type="hidden" name="action" value="START_PRODUCTION" /><Button variant="secondary"><Play size={16} /> Start production</Button></form> : null}
                {canManageProduction && request.status === CustomerProductionRequestStatus.IN_PRODUCTION ? <form action={advanceCustomerProductionAction}><input type="hidden" name="requestId" value={request.id} /><input type="hidden" name="action" value="MARK_READY" /><Button><PackageCheck size={16} /> Mark ready & notify customer</Button></form> : null}
                {request.status === CustomerProductionRequestStatus.READY ? <>{!request.balancePaidAt ? <FeedbackState state="warning" title="Ready but balance remains" description="The customer can pay the remaining verified balance from their request page. Completion is blocked until full payment." /> : null}{canManageProduction ? <form action={advanceCustomerProductionAction}><input type="hidden" name="requestId" value={request.id} /><input type="hidden" name="action" value="COMPLETE" /><Button disabled={!request.balancePaidAt}><CheckCircle2 size={16} /> Complete & notify customer</Button></form> : null}</> : null}
