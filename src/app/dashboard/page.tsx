...
export default async function DashboardPage({ searchParams }: DashboardPageProps) {
  ...
  const salesData = await Promise.all(
    days.map(async (day) => {
      ...
      return { label: day.toLocaleDateString("en", { weekday: "short" }), sales: Number(total._sum.totalAmount ?? 0) };
    }),
  );
    ...
    <div className="space-y-5">
      ...
      <section className="rounded-xl bg-slate-950 p-4 text-white shadow-xl sm:p-5">
        ...
        <div className="mt-5 grid grid-cols-2 gap-2 xl:grid-cols-4">
          {quickActions.map((action) => { const Icon = action.icon; return <Link key={action.href} href={action.href} className="group min-w-0 rounded-lg bg-white/10 p-3 transition hover:bg-white/15 sm:p-4"><div className="flex items-center justify-between"><Icon size={20} className="text-emerald-300" /><ArrowRight size={16} className="text-white/40 transition group-hover:translate-x-1" /></div><p className="mt-3 text-sm font-semibold sm:mt-4 sm:text-base">{action.label}</p><p className="mt-1 truncate text-xs text-white/55">{action.note}</p></Link>; })}
        </div>
      </section>
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Today's sales" value={currency(metrics.todaySales._sum.totalAmount?.toString() ?? "0", shop.currency)} helper={`${metrics.todaySales._count} transaction(s) today`} icon={<ShoppingBag size={20} />} />
        ...
        <StatCard label="Open debt" value={currency(Number(metrics.openDebts._sum.principalAmount ?? 0) - Number(metrics.openDebts._sum.paidAmount ?? 0), shop.currency)} helper={`${metrics.openDebts._count} active account(s)`} icon={<CreditCard size={20} />} />
        <StatCard label="Cash holds" value={String(metrics.cashHolds)} helper="Online cash reservations still active" />
      </section>

      <section className="grid gap-5 xl:grid-cols-[1.3fr_0.7fr]">
        <div className="panel p-4 sm:p-5">
          <div className="mb-4 flex items-center justify-between gap-3"><div><h2 className="text-lg font-semibold">Sales movement</h2><p className="text-sm text-slate-500">Last 7 days, scoped to {shop.name}</p></div><Badge tone="green">Tenant filtered</Badge></div>
      ...
      <section className="panel overflow-hidden">
        <div className="border-b border-[#ded8cd] p-4 sm:p-5"><h2 className="text-lg font-semibold">Recent orders</h2><p className="text-sm text-slate-500">Cashier, custom order, and online activity land here.</p></div>
