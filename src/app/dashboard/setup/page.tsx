  ...
  return (
    <div className="space-y-5">
      ...
      <section id="step-4" className={`panel scroll-mt-24 p-5 ${currentStep === 4 ? "ring-2 ring-cyan-400" : ""}`}>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">{OPTIONAL_BUSINESS_MODULES.map((module) => { const enabled = businessModuleEnabled(shop.enabledModules, module.key); return <div key={module.key} className={`rounded-xl border p-4 ${enabled ? "border-emerald-200 bg-emerald-50" : "border-slate-200 bg-slate-50"}`}><div className="flex items-center justify-between gap-3"><strong>{module.label}</strong><Badge tone={enabled ? "green" : module.status === "PLANNED" ? "orange" : "neutral"}>{enabled ? "Enabled" : module.status === "PLANNED" ? "Planned" : "Not enabled"}</Badge></div><p className="mt-2 text-xs leading-5 text-slate-600">{module.description}</p></div>; })}</div>
        <form action={reviewOnboardingModulesAction} className="mt-4 flex flex-wrap gap-3"><Button>Modules reviewed</Button><Link className="inline-flex min-h-11 items-center justify-center rounded-lg border border-slate-300 bg-white px-4 text-sm font-bold text-slate-800 transition hover:bg-slate-50" href="/dashboard/subscription">Review plan and usage</Link></form>
      </section>
      ...
      <section id="step-8" className={`panel scroll-mt-24 p-5 ${currentStep === 8 ? "ring-2 ring-cyan-400" : ""}`}>
        ...
        <div className="mt-4 flex flex-wrap gap-3"><Link className="inline-flex min-h-11 items-center justify-center rounded-lg border border-slate-300 bg-white px-4 text-sm font-bold text-slate-800 transition hover:bg-slate-50" href="/dashboard/staff">Open staff directory</Link><form action={reviewOnboardingStaffAction}><Button>Staff reviewed</Button></form></div>
      </section>
      ...
      <section id="step-9" className={`panel scroll-mt-24 p-5 ${currentStep === 9 ? "ring-2 ring-cyan-400" : ""}`}>
        ...
        {productCount ? <div className="mt-4 flex flex-wrap items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4"><Check className="text-emerald-700" /><span className="font-semibold">First item requirement complete.</span><Link className="inline-flex min-h-11 items-center justify-center rounded-lg border border-slate-300 bg-white px-4 text-sm font-bold text-slate-800 transition hover:bg-slate-50" href="/dashboard/catalog">Manage items</Link></div> : <form action={createProductAction} className="mt-4 grid gap-3 md:grid-cols-2">
