                ...
                <article key={debt.id} className="grid min-w-0 gap-4 p-3 sm:p-4 lg:grid-cols-[1fr_280px]">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <h3 className="min-w-0 truncate font-semibold">{debt.customer.name}</h3>
                      <Badge tone={debt.status === "PAID" ? "green" : isOverdue ? "red" : "orange"}>{isOverdue ? "Overdue" : titleCase(debt.status)}</Badge>
                    </div>
                    <p className="mt-1 text-sm leading-6 text-slate-500">Balance <strong className="text-slate-800">{currency(balance, shop.currency)}</strong> of {currency(debt.principalAmount.toString(), shop.currency)} · due {shortDate(debt.dueDate)}</p>
                    <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
                      {debt.installments.map((installment) => (
                        <span key={installment.id} className="shrink-0 rounded-lg bg-[#f6f4ef] px-2.5 py-1.5 text-xs font-semibold text-slate-600">{currency(installment.amount.toString(), shop.currency)} · {shortDate(installment.dueDate)}</span>
                      ))}
                    </div>
                    {debt.notes ? <p className="mt-3 break-words text-sm text-slate-600">{debt.notes}</p> : null}
                    ...
                    <div className="grid grid-cols-[repeat(2,minmax(0,1fr))] gap-2">
                      {[NotificationChannel.SMS, NotificationChannel.WHATSAPP].map((channel) => (
                        <form key={channel} action={sendDebtReminderAction}>
                          ...
                          <Button variant="outline" className="w-full px-2"><MessageSquareText size={15} /> {channel}</Button>
                        </form>
                      ))}
                    </div>
                  </div>
