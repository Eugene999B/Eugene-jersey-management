...
async function productionSettingsSession() {
...
async function currentLibrary(shopId: string) {
  const shop = await prisma.shop.findFirst({ where: { id: shopId }, select: { productionSetup: true } });
...
async function persistLibrary(input: { shopId: string; userId: string; setup: Parameters<typeof productionLibraryJson>[0]; library: ReturnType<typeof readProductionLibrary>; action: string; entityId?: string; metadata?: Record<string, unknown> }) {
...
}).refine((value) => value.maximumTemperatureC >= value.minimumTemperatureC, { message: "Maximum temperature must be at least the minimum." });

export async function saveHeatPressProfileAction(formData: FormData) {
  const { session, shopId } = await productionSettingsSession();
  ...
  const current = await currentLibrary(shopId);
  ...
  await persistLibrary({
    ...
    metadata: { name: parsed.data.name, plateWidthMm: parsed.data.plateWidthMm, plateHeightMm: parsed.data.plateHeightMm },
  });
...
export async function saveProductionMaterialAction(formData: FormData) {
  ...
  const item: ProductionMaterialSpec = {
    id: existing?.id ?? parsed.data.id || randomUUID(),
  ...
  if (current.library.materials.some((candidate) => candidate.id !== item.id && candidate.name.toLowerCase() === item.name.toLowerCase())) {
  ...
  await persistLibrary({
    ...
    entityId: item.id,
    metadata: { name: item.name, type: item.type, colour: item.colour, rollWidthMm: item.rollWidthMm, mirrorRequired: item.mirrorRequired },
  });
...
export async function saveProductionGarmentAction(formData: FormData) {
  ...
  if (!item.sizes.length) redirect("/dashboard/designs/materials?error=garment-sizes");
  if (current.library.garments.some((candidate) => candidate.id !== item.id && candidate.name.toLowerCase() === item.name.toLowerCase())) {
...
export async function saveProductionPlacementAction(formData: FormData) {
  ...
  if (current.library.placements.some((candidate) => candidate.id !== item.id && candidate.name.toLowerCase() === item.name.toLowerCase())) {
  ...
  await persistLibrary({
    ...
    entityId: item.id,
    metadata: { name: item.name, location: item.location, garmentId: item.garmentId || null, defaultWidthMm: item.defaultWidthMm, defaultHeightMm: item.defaultHeightMm },
  });
...
export async function setProductionResourceActiveAction(formData: FormData) {
  const { session, shopId } = await productionSettingsSession();
  const parsed = archiveSchema.safeParse({ resource: formData.get("resource"), id: formData.get("id"), nextActive: formData.get("nextActive") });
