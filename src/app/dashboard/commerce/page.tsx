  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold">Commerce control centre</h1>
        ...
        <div className="panel p-5">
          ...
          <form action={createCouponAction} className="grid gap-3 md:grid-cols-2">
            ...
            <select className="field" name="discountType" defaultValue={CouponDiscountType.PERCENT}>
              {Object.values(CouponDiscountType).map((type) => <option key={type} value={type}>{titleCase(type)}</option>)}
            </select>
            ...
            <Button>Create or update coupon</Button>
          </form>
          <div className="mt-4 grid gap-2">
            {coupons.map((coupon) => (
              <div key={coupon.id} className="flex items-center justify-between rounded-[8px] bg-white px-3 py-2 text-sm">
                ...
                <span className="text-slate-600">
                  {coupon.discountType === "PERCENT" ? `${coupon.value}%` : currency(coupon.value.toString(), shop.currency)} | used {coupon.usedCount}{coupon.usageLimit ? ` of ${coupon.usageLimit}` : ""}
                </span>
              </div>
            ))}
